...
{
  mPrint("Json Dwn :: " + String((char *)buf));
  led_show = true;
  Rx_cont++;
  Rx_last_time = sys_time;
...
#		endif //_MONITOR
  ...
  //  Serial.println(freqs[gwayConfig.ch].dwnFreq);
...
int buildPacket(uint8_t *buff_up, struct LoraUp *LoraUp, bool internal)
{
  ...
  //  int i;
  //  for (i = 0; i < LoraUp->payLength; i++) {
  //    Serial.print(LoraUp->payLoad[i], HEX);
  //    Serial.print(' ');
  //  }
...
#	endif //_MONITOR
  ...
  buff_up[7]  = MAC_array[6];
  buff_up[8]  = MAC_array[7];
  ...
  // -----------------
  sprintf(cfreq, "%.6f", (double)freqs[gwayConfig.ch].upFreq / 1000000);
...
#	endif
  mPrint("Json UP :: " + String((char *)(buff_up + 12)) + " , length=" + String(buff_index));
  return (buff_index);
